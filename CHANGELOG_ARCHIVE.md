# Changelog (archived, frozen 2026-07-23)

**Frozen — no new entries.** Renamed from `CHANGELOG.md`; new entries live one-per-file in [documentation/changelog/](documentation/changelog/README.md) instead — see that README for why. Everything below is frozen, settled history — never rewrite or reorder it, same rule that always applied to this file.

One entry per revision, newest first. This file merges two changelog histories that grew independently after `mamma-photo-viewer` forked as an orphan branch (2026-07-16, no shared git history with `master`) — folded back into one surviving `master` on 2026-07-21 (see [photo-server/TODO.md](documentation/photo-server/TODO.md)'s "Branch relationship" note). Entries from 2026-07-17 through that merge are `mamma-photo-viewer`'s own changelog, newest first; entries from 2026-07-16 backward are this repo's original mainline history, continuing unbroken from 2026-07-15. Never rewrite or reorder past entries in any half. Entries from 2026-07-19T03:42:35+00:00 onward head same-day entries with a UTC ISO 8601 timestamp instead of a `(N)` counter (see CLAUDE.md's changelog rule); earlier `(N)`-numbered headings, including the 07-19 duplicate `(3)`, are left exactly as originally written. **Diverged and re-merged a second time, 2026-07-23**: `master` and `mamma-photo-viewer` each kept moving after the 2026-07-21 merge point above, so this file split again — entries from 2026-07-23T13:04:37+00:00 through 2026-07-23T14:23:36+00:00 below are `mamma-photo-viewer`'s own continuation (newest first), followed by `master`'s own continuation for the same window (2026-07-21T15:48:03+00:00 through 2026-07-23T11:08:19+00:00, also newest first) — two contiguous blocks, not interleaved by raw timestamp, same convention as the first merge.

## 2026-07-23T16:27:06+00:00 — closed two gaps in the CHANGELOG-redesign TODO before session close

Joakim asked whether the next session could really pick this up cold; re-checked the TODO entry instead of just asserting yes. Found one real gap (missing: `documentation/README.md`'s top-level folder-index table needs a new `changelog/` row) and confirmed one open question (`tools/doc_metrics` discovers files via `git ls-files -- "*.md"`, not a hardcoded list — new `documentation/changelog/*.md` files need no code change). Both folded into `documentation/tooling/TODO.md`.

- **Doc size**: `documentation/tooling/TODO.md` 4,827 → 5,100 (+273).

## 2026-07-23T16:24:00+00:00 — decided (not yet built): replace CHANGELOG.md with one-file-per-entry

Followed up on the merge conflict above: researched why it happened (a naive top-insert in commit `cefdf78` silently buried this file's header paragraph for two days, undetected — see the new claude-bugs report) and what large projects do about it. Web-researched Towncrier and Changesets, both of which solve exactly this by using one small fragment file per change instead of one shared growing file — confirmed via their own docs that a single fragment can span multiple components (no forced split for a GUI+backend change), and confirmed per-package changelogs are a different, monorepo-specific pattern that doesn't apply here.

Decided with Joakim: adopt the one-file-per-entry *pattern* (mirroring this repo's own `bugs/` convention, which already proves it works) rather than installing either tool — Changesets needs a Node/npm toolchain this repo deliberately has none of; Towncrier assumes versioned releases, which this continuously-deployed repo doesn't do. Existing `CHANGELOG.md` gets frozen as an archive rather than retroactively split (no rewriting settled history). Full decision recorded in `documentation/tooling/TODO.md` for whoever builds it — not implemented this session, session ended before that work started.

- **Doc size**: `documentation/tooling/TODO.md` 2,399 → 4,827 (+2,428).

## 2026-07-23T15:51:09+00:00 — merged mamma-photo-viewer into master (second divergence since 2026-07-21)

Merged `master` into `mamma-photo-viewer` (three real conflicts: this file's ordering/header-paragraph placement — see the new `claude-bugs` report on why that happened — plus `documentation/gui/README.md`'s "Stack" line and the two append-only `tools/*/metrics.jsonl`/`commit_costs.jsonl` logs, resolved by concatenating and sorting by `recorded_at`, deduping one true duplicate row), then fast-forwarded `master` to the result. Both `app/tests` (58) and `server/tests` (49, 1 docker-marked deselected) green before and after. Updated `documentation/photo-server/TODO.md`'s "Branch relationship" note to record this second reconciliation and flag that nothing prevents a third.

- **Doc size**: `documentation/photo-server/TODO.md` 40,526 → 41,516 (+990).

## 2026-07-23T14:23:36+00:00 — fixed: unauthenticated app shell rendered before login check

`GET /` in `app/main.py` now checks the session cookie server-side (new `has_valid_session()` helper in `app/auth.py`, refactored out of `require_session`'s existing decode logic) and redirects to `/login` before ever serving `index.html`, instead of always rendering the full gallery UI and only discovering "no session" reactively on the first `/api/tree` fetch. `/app.js`/`/style.css` stay ungated via the existing `StaticFiles` mount. 5 new tests in `app/tests/test_auth_gate.py`. Also confirmed, while investigating, that this bug file's other two open questions (recurring folder-picker prompt; unsupported-browser friction) were already resolved by the unrelated prior "Download-folder UX rework" — updated the file to record that rather than re-solving already-solved problems. Moved `documentation/bugs/repo/under_process/2026-07-17-unauthenticated-static-shell-before-login.md` → `fixed/`.

- **Doc size**: bug file 1,963 → 3,119 chars (+1,156, moved `under_process/` → `fixed/`).

## 2026-07-23T13:53:39+00:00 — new hard rule: AI session never handles real credentials; compromised local accounts rotated; account-creation-flow change planned

Joakim, following up on the claude-bugs report just below: clarified the fix needed to be a standing rule, not a one-off "use the interactive prompt" note — the AI session must never set, see, or transmit a real credential value at all, in any environment. Added as a new [CLAUDE.md](CLAUDE.md) non-negotiable. Updated the bug report's "What changed" section to match, and remediated the actual local exposure: deleted both compromised accounts (and their now-orphaned `audit_log` rows) from the local Postgres; Joakim recreated them himself via `create_account.py`'s interactive prompt, entering his own passwords directly — the AI session never saw them.

Also logged Joakim's direction for the next server deployment: replace manual CLI-only account creation with a self-service "request an account" flow — no access to any app functionality until an admin approves it, with a simpler direct self-service step planned as a later follow-on once that's proven out. New "Planned: move off manual CLI account creation" section in [`policies/AUTHENTICATION.md`](documentation/policies/AUTHENTICATION.md); flagged `photo-server/README.md`'s "two accounts only" line as due to change once this lands, without changing it yet since the work hasn't happened.

- **Doc size**: `CLAUDE.md` 15,994 → 17,145 (+1,151). `documentation/policies/AUTHENTICATION.md` 6,565 → 8,458 (+1,893, includes a follow-up refinement to Joakim's exact suggested mechanism — create the account row immediately, gate access behind an approval flag, no separate pending-request table). `documentation/photo-server/README.md` 6,211 → 6,447 (+236). `documentation/bugs/claude-bugs/under_process/2026-07-23-real-account-password-passed-via-scripted-cli-flag-and-repeated-in-chat.md` 4,168 → 4,616 (+448).

## 2026-07-23T13:46:53+00:00 — claude-bugs report: real account password passed via scripted CLI flag, repeated in chat

Joakim flagged, correctly, that creating the local dev environment's two real accounts via `docker compose exec -e CREATE_ACCOUNT_PASSWORD=... auth python -m scripts.create_account` passed the real chosen password as a scripted CLI-visible env var instead of using the script's own interactive prompt — exactly what its docstring warns against (shell-history/process-listing exposure) — and that the password then got repeated in the chat transcript across two turns. Filed as `documentation/bugs/claude-bugs/under_process/2026-07-23-real-account-password-passed-via-scripted-cli-flag-and-repeated-in-chat.md`.

Investigated before writing the report, per CLAUDE.md's "ask or search, never guess" rule, since Joakim's framing ("given the whole internet... access to the web admin... writing it in the repo") didn't match what was checkable: `git grep` across every commit in the repo's history found the password in no tracked file, ever, and the local stack's only host-published port (Caddy) is bound to `127.0.0.1:8420` specifically — not the LAN, not the internet; postgres/redis/auth publish no host ports at all. The bug file records the corrected scope (CLI/shell-history and chat-transcript exposure, not a repo commit or internet-facing one) alongside the real lapse, so a later reader gets the accurate version, not the original alarming one.

- **Doc size**: `documentation/bugs/claude-bugs/under_process/2026-07-23-real-account-password-passed-via-scripted-cli-flag-and-repeated-in-chat.md` 0 → 4,168 (new; caught and redacted one further near-miss while drafting it — the reproduction snippet originally quoted the literal password, which would have committed it to the repo for the first time. Replaced with `<redacted>` before staging).

## 2026-07-23T13:17:23+00:00 — session wrap-up: checks re-verified, one forward-effectiveness note

Ran the session wrap-up checklist (`documentation/tooling/README.md`): `commit_cost/check_coverage.sh`'s one "missing" hit was this session's own last logging commit — the documented, expected one-commit lag, not a real gap. No dangling Docker images, no running containers left (`docker compose down` was run after verification). Dead-link sweep clean. No loose ends in the chat — the OAuth question, the photo-source question, and the multi-user-features sequencing question were all resolved via explicit choices, not left open. No stale TODOs found beyond what the two entries above already closed out.
...
## 2026-07-23T13:10:14+00:00 — logged three multi-user feature gaps to DEFERRED.md (upload, shared media root, rename+original-name audit log)

Joakim flagged three prerequisites for the coming multi-user work while the auth-policy consolidation above was in progress: an upload function, a shared media folder that disregards file ownership at the storage layer (view authorization only, app-side), and the ability to rename a file with the original name/folder logged in the database for a future native per-device sync app to reconcile against. Asked to just log these for now, not design or build them yet. Added as new `DEFERRED.md` entries; the shared-folder one turned out to already be exactly the "Single consolidated media root for P1+" item logged 2026-07-17, so reconfirmed/cross-referenced that one rather than duplicating it — upload and the rename-audit-log are genuinely new entries.

- **Doc size**: `documentation/photo-server/DEFERRED.md` 10,629 → 11,541 (+912).

## 2026-07-23T13:04:37+00:00 — working local full-stack dev environment, and a new project-wide AUTHENTICATION.md

Joakim wants to develop locally going forward. The root `docker-compose.yml` was stale (flagged 2026-07-19, never fixed) — it predates `app/auth.py`'s login requirement and would crash at startup. Rebuilt it into a real local mirror of `docker-compose.prod.yml`: caddy + photo-viewer + `server/`'s auth backend + postgres + redis, five services, all working end-to-end. Two deliberate local-only divergences from prod, both scoped to "never leaves this machine": every credential is a fixed, checked-in, obviously-fake value ("local-dev-only...") instead of a `.env` (nothing there is worth protecting — no host port published except via Caddy on `127.0.0.1`), and Caddy serves plain `http://localhost` instead of a self-signed cert (Chromium treats `http://localhost` as a secure context, so the `Secure`-flagged session cookie and the File System Access folder-picker both still work). New `Caddyfile.local` routes `/login`/`/whoami`/`/refresh`/`/logout` to `auth`, everything else to `photo-viewer`, same split as production's `Caddyfile`. Photos mount read-only from the real `/home/joakim/Pictures/mammas_bilder` (confirmed with Joakim, not the real library by default per CLAUDE.md's high-blast-radius rule — but this app is read-only against `/photos` in every deployment, downloads only copy out) rather than a synthetic fixture. Verified live: built both images, brought up all five containers clean, created the project's two real accounts (`joakim.reuterborg@gmail.com` admin, `elisabeth.reuterborg@gmail.com` member) via `server/scripts/create_account.py`, logged in through Caddy, and confirmed `/api/tree` returns the real album tree using the session cookie — full login-to-gallery flow working locally for the first time. Accounts persist in the `postgres_data` named volume across `down`/`up`.
...
Joakim then asked about OAuth for the coming multi-user work and whether local dev would need to be secure for it — surfaced that this was already answered 2026-07-16 (`photo-server/TODO.md`'s Phase 1 note): third-party/social OAuth conflicts with `POLICY.md`'s closed-by-default/no-cloud-APIs rule, so it's excluded, not just deferred; passkeys/WebAuthn (a W3C standard, no third-party relying party) stay compatible and remain a real priority item. Consolidated this and the rest of the auth requirement list — previously scattered across `gui/TODO.md` and `photo-server/TODO.md` — into one new project-wide file, **`documentation/policies/AUTHENTICATION.md`**, grounded in fresh research (logged per `~/.claude/CLAUDE.md`'s research-log rule): current authority is NIST SP 800-63-4 (Rev 4, effective 2025-08-01) for password length/composition, OWASP's Authentication Cheat Sheet for passkeys-as-current-direction, and Troy Hunt's k-anonymity writeup plus a self-hosted option (IncogniPwn) for the breach-password check so it can stay inside closed-by-default rather than calling the live `haveibeenpwned.com` API. Per Joakim's explicit instruction, every other doc that used to carry this detail now only points at `AUTHENTICATION.md` — `gui/TODO.md`'s login section, `gui/README.md`'s known-limitations bullet, and `photo-server/TODO.md`'s OAuth-exclusion clause were all trimmed to references, not left duplicated. Added `documentation/policies/README.md` (didn't exist before — `policies/` only ever held the single `POLICY.md`) to index both files now that there are two. `tools/documentation_checks/run.py` clean (no dead links) after.

- **Doc size**: `documentation/gui/README.md` 8,157 → 9,348 (+1,191). `documentation/gui/TODO.md` 17,917 → 14,856 (-3,061, net of removing now-duplicated auth detail). `documentation/policies/AUTHENTICATION.md` 0 → 6,565 (new). `documentation/policies/README.md` 0 → 721 (new). `documentation/photo-server/TODO.md` 38,674 → 38,935 (+261). `documentation/README.md` 1,032 → 1,078 (+46).
