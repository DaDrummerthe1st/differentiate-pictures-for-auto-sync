...
**Generalized to every feature, not just tagging — raised 2026-09-07**: "the user should always decide" is a standing rule across the whole app, not scoped to curation/tagging. Presets and recommended defaults are encouraged and should be easy to reach ("correct information at the right place at the right time"), but never mandatory or silently applied — any feature (sync timing, sort order, what gets flagged for removal, how aggressively storage gets reclaimed) is something a user opts into because it demonstrably works for her, not something the app decides on her behalf. Confirmed with Joakim answering a NAS-sync design question, but explicitly stated as general, not narrow to that feature.

**Non-invasive usage-pattern learning as a core, near-top-priority feature — raised 2026-09-07**: Joakim's own words, calling this "almost the most important area" and the source of the app's real USPs — an early need for pilot users and a way to learn, without being invasive, how each individual actually uses the app's features. This isn't a new concept to design from scratch: it's the same thing [curation/IDENTITY_MATCHING.md](curation/IDENTITY_MATCHING.md) already calls "incidental/behind-the-scenes" usage signal (which photo of a burst a user keeps, what she searches then acts on) — arrived at independently from the pilot-user/USP angle rather than the scoring-mechanism angle that doc took. Worth treating as a first-class rollout concern, not just a scoring detail, once pilot users are a real target — see [distributed-sync/TODO.md](distributed-sync/TODO.md)/rollout planning for where that lands.
...
**iOS parity considered at every design decision, raised 2026-09-07**: Android is the only implementation target today, but every feature/architecture decision should ask "how would this be solved on iOS" and "would this work the same way" — not to build for iOS now, but so today's Android-only choices don't quietly foreclose an iOS version later. Concretely resolved that same day: whether the client is native Kotlin or React **Native** doesn't change this much — either is a real mobile app with background/closed-app photo-library access, and cross-platform frameworks like React Native still need platform-specific native modules for the deepest OS integration regardless, so switching frameworks doesn't buy a fundamentally easier path to iOS on its own. What *would* matter is React as a **web app/PWA** instead — that reintroduces the exact structural limitation ([curation/IDENTITY_MATCHING.md](curation/IDENTITY_MATCHING.md)'s "Reversed 2026-09-05" note) the native pivot escaped, on both platforms, not a framework-choice nuance. Decision: stay on native Kotlin/`android-app-test1` — see [mobile/README.md](mobile/README.md).
