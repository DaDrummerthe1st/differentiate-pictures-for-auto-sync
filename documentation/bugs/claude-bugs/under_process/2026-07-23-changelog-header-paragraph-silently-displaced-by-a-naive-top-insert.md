...
Merging `master` into `mamma-photo-viewer` (2026-07-23) produced a `CHANGELOG_ARCHIVE.md` with two problems, neither flagged as a conflict by `git merge`:
...
- **No structural check added yet** — this bug's root cause (a silent top-insert past a structural paragraph) is still fully reproducible by the next `CHANGELOG_ARCHIVE.md` edit, by hand or by a future AI session, since nothing enforces "header paragraph stays at the top" or "entries stay in descending timestamp order." Worth a lightweight check (e.g. in `tools/documentation_checks`) if this recurs.
