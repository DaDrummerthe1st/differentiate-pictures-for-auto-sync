...
Traced with `git show <commit>:CHANGELOG.md` and `git log -S` across both branches' history (the file's name at every point in this investigation — it wasn't renamed to `CHANGELOG_ARCHIVE.md` until this bug's own fix, see "What changed" below):
