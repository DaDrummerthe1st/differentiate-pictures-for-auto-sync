...
| Exposure | Over/under-exposed | Same bucket as blur for a "technically bad photo" cluster | built — `detector/quality.py`'s `detect_exposure` (categorical, deployed). `modules/quality.py`'s standalone `check_exposure` reports the same signal as -100 to +100 (EV-style, 0=balanced). |
| Monochrome/low-colour | Black-and-white or washed-out ("pocket-shot") | Already named in [../tags/TAXONOMY.md](../tags/TAXONOMY.md)'s quality category | built — `detector/quality.py`'s `detect_monochrome` (boolean, deployed). `modules/quality.py`'s standalone `check_saturation` reports the same signal as 0-100%. |
...
| --- | --- | --- | --- |
| Face detection | Where faces are (box) | Feeds bounding-box tagging, [../tags/UX_FLOWS.md](../tags/UX_FLOWS.md) | built — `detector/faces.py`'s `detect_faces` |
...
**Picks (researched 2026-08-02)**: **face detection** — YuNet (OpenCV Zoo, Apache-2.0, sub-1MB ONNX, millisecond CPU cost, ships free via OpenCV's built-in `FaceDetectorYN`, already returns 5 landmarks). **Built 2026-08-07** (`documentation/curation/TODO.md`'s build-plan Phase 3): the model's own `LICENSE` file, checked directly rather than assumed (`detector/models/LICENSES.md`), is actually **MIT** (Shiqi Yu) — the repo root's Apache-2.0 covers `opencv_zoo`'s own code, not each vendored model asset; still within this project's MIT/Apache-2.0-only bar either way. **Face recognition/embedding** — MobileFaceNet's standalone MIT-licensed ONNX release (~1M params, 128-d embedding) over InsightFace's `buffalo_s` bundle, specifically to sidestep InsightFace's pretrained-weights license (code is MIT, but the weights themselves are non-commercial-research-only per their own policy — a gray area for a private but non-"research" family server, worth a conscious call rather than an assumption). **Correction, 2026-09-09** (on-device Android build): the actual vendored file (`arcface_mobilefacenet.onnx`, see `android/app/src/main/assets/models/LICENSES.md`) outputs a **512-d** embedding, not 128-d — that figure described MobileFaceNet variants generally, never checked against this exact file until now. Doesn't change the pick, just the dimension to code against. **Emotion** — FER+ (`emotion-ferplus-8`, Apache-2.0, ready-made ONNX file from the ONNX Model Zoo) is the *only* genuinely turnkey pretrained option found in this category; everything else surveyed is research code needing your own training/export.
...
| Area | What it detects | Why it matters | Status |
| --- | --- | --- | --- |
| EXIF/GPS extraction | Where/when a photo was taken | Already built ([../../previous-work/multi-user-web-app/prototypes/differentiate_pictures/app/gpsdata.py](../../previous-work/multi-user-web-app/prototypes/differentiate_pictures/app/gpsdata.py) — repointed again 2026-09-05 when `archive/` moved to `previous-work/multi-user-web-app/` in the native-app pivot; see `previous-work/README.md`) | done (pre-existing, now archived) |
