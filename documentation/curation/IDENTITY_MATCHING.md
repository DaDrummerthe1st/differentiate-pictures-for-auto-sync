...
**Sync scope structure — confirmed 2026-09-07**: default top-level split is **pictures | metadata**, each further divided into subgroups/sub-subgroups for granular per-item sync decisions — the concrete shape "the user always decides" (above) takes for this feature specifically.

**No separate app — embedding vs. building the sync engine, real fork raised 2026-09-07, not decided**: Joakim was clear a user must never be asked to install a second app to get sync — "we clean our own messes." Researched: this doesn't require a separate app either way. Syncthing's core is a Go library; a real community project (**SyncUp**, not official) proves it can be compiled via Go's `gomobile` tool into an Android `.aar`/iOS framework and called directly from Kotlin/Swift, invisible to the user as anything but a DPFAS feature. **Maintenance check, since it matters for a "reuse a mature engine" argument**: the *core* is actively maintained by the Syncthing Foundation (a Swedish non-profit) — v2.1.3 released 2026-08-05, v2.1.2 2026-07-08 — genuinely healthy, distinct from the separately **discontinued** official Android GUI-wrapper app (last release Dec 2024, just one convenience client around the engine, not the engine itself). **Left open, not decided**: embed Syncthing's engine (reuses a mature, battle-tested sync/conflict-resolution implementation, but via unofficial gomobile bindings with no official SDK — real integration work) vs. build a simpler custom protocol tailored exactly to this app's needs (full ownership, no third-party Go engine, but reinvents conflict resolution/discovery and its own security surface from scratch). **Joakim wants a dedicated, thorough security research pass before deciding** — MITM risk, leak surface, protocol-level review of both the embed and build-custom paths — explicitly too large for this session, flagged as its own future research item, not attempted here.

**iOS interim decision, 2026-09-07**: given iOS suspends networking once an app is backgrounded (confirmed via Apple's own developer forums: Multipeer Connectivity "stops browsing/advertising when backgrounded," a platform constraint no sync-engine choice avoids), the interim answer is **the iOS app must stay open/foreground until a sync completes** — revisit once there's real experience with how iOS users tolerate that, not designed further now.
