...
- **Tags — built 2026-08-05**: a narrowed, build-ready slice of [../tags/TAXONOMY.md](../tags/TAXONOMY.md)'s full 12-category design (people/places/objects/animals + a free-text catch-all; the other 8 categories are out of scope for now). Own SQLite table (`tags`, in the same analytics DB), keyed by photo path + user rather than a Postgres `photo_id` this app has no catalog for — see [../tags/SCHEMA.md](../tags/SCHEMA.md)'s "Now (app/'s lightweight build)" section for why and how this differs from the fuller Postgres design. In the lightbox: an "🖇️ Tagga" button adds a whole-photo tag (e.g. place = "the beach"); dragging directly on the photo draws a bounding box (e.g. a person = "mother"), which opens the same form. Existing tags render as chips (whole-photo) or boxes-with-labels (bounding-box) overlaid on the image, click any to edit or delete. Value autocompletes against the user's own past tags in that category (`GET /api/tags/values`) — a lightweight stand-in for the full design's `entities` table. Every tag row carries a `source` column (`manual` today; `auto` is a reserved value for the automatic-tagging work planned for the next session — nothing produces it yet, but the display/edit UI already renders it distinctly, subtler/dashed, per [../tags/UX_FLOWS.md](../tags/UX_FLOWS.md)'s "subtle, concealable" rule for detector-sourced boxes).
