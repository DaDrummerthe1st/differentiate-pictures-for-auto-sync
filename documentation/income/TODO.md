...
- **Phone↔NAS connectivity infra cost, raised 2026-09-06, genuinely open**: Joakim's rule is that basic phone-to-own-NAS connectivity must always be free — clear for buyers of this project's own hardware (the cost presumably folds into the hardware sale, same shape as the bullet above). Not yet decided whether that same free connectivity extends indefinitely to someone running only the FOSS server software on their own hardware, since operating the coordination server ([../distributed-sync/README.md](../distributed-sync/README.md)'s "Phone ↔ NAS connectivity" section) at scale is a real ongoing cost, and per Joakim's own words "no costs can come up without an income." A relay/TURN-equivalent tier for users whose network setup can't connect directly (CGNAT etc.) is the one piece already agreed as paid-and-opt-in, not free by default.
