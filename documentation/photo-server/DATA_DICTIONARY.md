...
| id | pk | now |
| email | unique | now |
| username | unique, opaque random token (not a real name — see [../GLOSSARY.md](../GLOSSARY.md)'s "Opaque token" entry), generated by `create_account` | now, added 2026-08-12 for per-user storage scoping (`dpfas_media/<username>/`), see [../plans/deep-singing-firefly.md](../plans/deep-singing-firefly.md) |
