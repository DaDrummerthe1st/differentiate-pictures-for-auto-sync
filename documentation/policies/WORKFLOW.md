...
**Not yet applied, real gap found 2026-09-06**: the native-app pivot added two more dependency ecosystems Dependabot already supports but this repo's `.github/dependabot.yml` doesn't yet cover — `npm` (the React Native app shell, [GLOSSARY.md](../GLOSSARY.md)'s app-shell decision) and `gradle`/Swift Package Manager (the Kotlin+Swift native modules). Handed to a separate cross-session dependency audit rather than applied here.

**Forward-looking, for whenever this project's own self-hosted-git plan (this file's "Vendor lock-in and openness" independence note) actually happens**: Dependabot is GitHub-exclusive and disappears entirely once this repo moves off GitHub — plan ahead of that, don't discover it at migration time. **A raw CVE feed subscription (e.g. NVD's) is the wrong shape of tool for this** — it's an unfiltered firehose of every CVE ever published, with no automatic cross-referencing against this repo's actual dependency list, so on its own it wouldn't tell you whether any given CVE even applies here. **Renovate**, the obvious host-agnostic Dependabot equivalent (works against self-hosted Gitea/Forgejo, not just GitHub), is disqualified by this project's own AGPL-3.0 exclusion rule ([GLOSSARY.md](../GLOSSARY.md)'s AGPL entry) — self-hosting it as a running bot against this project's own git server is exactly the "network service" scenario that license's obligation targets, not a low-risk edge case. Two real, license-clean alternatives verified 2026-09-06 instead: **OSV-Scanner in offline mode** (Apache-2.0-licensed tool, Google-authored but not tied to any single git host — downloads OSV's vulnerability database once, like an antivirus signature update, then every scan runs entirely locally with zero network calls, consistent with this project's closed-by-default rule) run as a scheduled job on whatever CI the self-hosted git server has; or **OWASP Dependency-Track** (Apache-2.0, OWASP Foundation-governed — a nonprofit, not a single company — self-hostable via Docker Compose) for a continuously-running dashboard instead of periodic scans, ingesting an SBOM and cross-referencing it against NVD/GitHub Advisories/OSV automatically. Neither is implemented now — flagged for whenever the self-hosted-git migration is actually scheduled.
