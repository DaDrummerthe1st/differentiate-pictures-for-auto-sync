...
**Central register, added 2026-09-06**: before adding any new external dependency — a Python package, an Android/Gradle library, a Docker base image, or a vendored AI model pick — check [security/DEPENDENCIES.md](../security/DEPENDENCIES.md) first. It's this repo's license/maintenance-health/CVE inventory, built from a full-repo audit, not a per-topic afterthought. **Update it in the same pass** whenever something changes it would need to know about (a new dependency, a version bump, a newly discovered risk, a resolved flag) — a register nobody updates decays exactly as fast as any other doc here, and skipping the update just means the next session re-discovers the same finding from scratch. A dependency already carrying a `DO NOT REMOVE THIS COMMENT UNTIL RESOLVED` marker in its own manifest/Dockerfile/compose file is a **confirmed, unresolved flag** — don't add new code that leans on it more heavily, and don't remove the comment without actually resolving what it names (replacing the dependency, or recording a deliberate, narrow exception in [POLICY.md](POLICY.md)).

**Automated complement, added 2026-08-07, coverage confirmed broken 2026-09-06**: the manual per-step check above doesn't run between sessions, so GitHub Dependabot (`.github/dependabot.yml`) is meant to be the always-on backstop — vulnerability alerts + automated security-fix PRs, checked weekly, notify-only by design (a human still reviews and merges, matching DETECTORS.md's "vendor deliberately" convention). **As of the 2026-09-06 dependency audit, this backstop covers almost nothing**: the config still names a `pip`/`/modules` path that no longer exists post-pivot (a prior archive commit deleted the working `pip`/`uv`/`docker` entries for what's now `previous-work/` instead of repointing them, and never added a `gradle` entry for `android/`) — see [security/DEPENDENCIES.md](../security/DEPENDENCIES.md) for the concrete fix, not yet applied (a config change needing Joakim's go-ahead). Don't assume Dependabot is catching anything in this repo until that's fixed. Delivery is email, via each GitHub account's own notification settings (Settings → Notifications → the Dependabot-alerts/security-updates toggles) — not something this repo's config can force on Joakim's behalf; worth him confirming those are checked once real coverage exists again. Scope, once fixed, would still only be this repo's own dependencies — the `.10` server's host OS packages (a separate `hardware` repo, outside this repo's visibility) are explicitly out of scope.
