...
**Forward-effectiveness note**: enabling Dependabot immediately surfaced 50 pre-existing vulnerabilities (`pillow` and `python-multipart`, both several versions behind and both fixed same-session) that a bare "check PyPI for the latest release" habit had been missing — a newest-version check confirms freshness, not vulnerability-freeness, and those aren't the same question. Going forward, the numbered-step dependency check above should include `gh api repos/<owner>/<repo>/dependabot/alerts?state=open` (or the repo's Security → Dependabot tab) wherever this repo's dependencies are touched, not just a PyPI-latest lookup — it names the exact vulnerable range and patched version directly, cheaper than inferring CVE exposure by hand.

## Debugging discipline

- **Trace every found bug/defect to its actual root cause** — which commit/change introduced it, what mechanism let it happen — not just patch the symptom. Decided 2026-07-23 after a structural CHANGELOG.md defect went unnoticed across several commits; a naive symptom-fix would have left the same failure mode reproducible by the next edit.
- **Never apply a fix from reasoning/theory alone** — confirm the actual root cause with real data (logs, instrumentation, a targeted diagnostic check) before making the change, not after it fails. If a plausible cause can't be cheaply confirmed, say so and propose the diagnostic step instead of shipping a guess framed as a fix. Decided 2026-09-06 after two theory-only changes in a row (Coil crossfade/cache-key guesses) neither confirmed nor actually fixed the symptom.
- **When a Claude Code permission prompt fires on a command that should already be allowlisted, check the actual configured settings/hooks first** (`.claude/settings.json`, `.claude/settings.local.json`, `~/.claude/settings.json` and any `PreToolUse`/`PostToolUse` hooks they reference) — don't theorize or delegate the diagnosis to a subagent with no more visibility into this session's own config than a generic docs lookup. A hook can emit its own `"permissionDecision": "ask"` fully independent of the Bash allowlist. See `documentation/bugs/claude-bugs/under_process/2026-09-05-mis-diagnosed-a-permission-prompt-and-edited-global-hook-config-before-verifying.md`.
- **Debugging a running app's GUI behavior**: use logs/instrumentation (logcat, `Log.d`, `dumpsys`, `uiautomator dump`), not screenshots — not even a single one-off check. For anything that genuinely needs eyes on the screen, ask Joakim to check and report back. Decided 2026-09-06.
- **Before writing a `BitmapFactory` bounds-only decode (`inJustDecodeBounds = true`) or any other code whose success signal isn't the call's own return value, check `documentation/bugs/` for a prior incident in the same shape** (`grep -ri` the relevant API/pattern) — this repo has hit the elvis-after-bounds-only-decode trap twice: `decodeStream` always returns `null` in bounds-only mode, so a naive `... ?: return null` right after it treats every success as a failure. Decided 2026-09-09 after repeating the already-fixed 2026-09-06 photo-grid version of this exact bug in new face-scan code, despite having read the original bug file earlier in the same session — see `documentation/bugs/claude-bugs/fixed/2026-09-09-repeated-the-2026-09-06-elvis-operator-bounds-decode-bug-in-new-on-device-face-scan-code.md`.
...
- **This repo has no isolation between concurrent Claude Code sessions sharing one working directory** — no `git worktree`, no per-session clone. A second session's `git checkout`/commit/merge activity can silently switch the shared branch or interleave commits under you. If git state or a pre-commit chain looks inexplicably wrong mid-session (unexpected branch, a chain that shouldn't be infinite, files you didn't touch), check `git branch --show-current` and use `ListAgents`/`SendMessage` to check for an active peer session *before* concluding it's a tooling bug. A peer can hand you its own concrete facts (branch, uncommitted state, its own instructions) directly. If a peer's answer reveals genuinely conflicting strategic direction (not just file overlap), surface it to Joakim via `AskUserQuestion` rather than assuming your own instruction wins because it's more recent — then relay the resolution back to the peer. For genuinely separate subsystem work with no file overlap, prefer a dedicated worktree/branch over sharing the checkout. Decided 2026-09-07, see `documentation/bugs/repo/fixed/2026-09-04-concurrent-sessions-sharing-one-working-directory-caused-branch-switch-confusion-and-a-false-infinite-commit-cost-catch-up-chain-SOLVED.md`.
- **Verify `HEAD` after every commit in a shared checkout, before reporting success or pushing.** A clean pre-commit hook run (secrets scan clean, ledger coverage OK, no error) is not proof your commit captured your own intended content under your own intended message — staging and committing both act on one shared index a peer session can mutate between your tool calls, including a peer's own commit sweeping up whatever you'd already staged, silently, with only an easily-missed `no changes added to commit` line to show for it. Run `git log -1 --format=%B` (and `git show --stat HEAD` if in doubt) and confirm it matches what you meant to commit; if it doesn't, investigate before pushing anything. Apply this with extra care right after a pause/resume — that gap is exactly where a peer's commit lands unseen. Decided 2026-09-11, see `documentation/bugs/claude-bugs/under_process/2026-09-11-staged-commit-swept-into-concurrent-session-s-commit-in-shared-checkout.md`.
