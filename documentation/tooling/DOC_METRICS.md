...
`log.py` records the *current* HEAD, so it can only be run after a commit exists — a commit's own snapshot always lands in a later commit, one commit behind. This isn't a workaround that could be fixed with more effort: a commit's hash is computed *from* its tree, so that tree can never already contain a row referencing the not-yet-computed hash of the commit it's part of — the same reason a file can't contain its own checksum computed in advance. This project never amends commits (see CLAUDE.md) either, so there's no way to close the gap after the fact. It's a permanent, structural one-line-per-commit lag, not something to chase.

**2026-09-06**: closed automatically by `.githooks/pre-commit` — every commit attempt logs whatever the *previous* commit is still missing and folds the result straight into the commit already being made, so the lag is always exactly one commit and never more (see [README.md](README.md)'s "Pre-commit hook" section). Before this date, `.githooks/post-commit` closed the gap instead, via a separate auto-generated catch-up commit — retired after it turned out to have a structural gap of its own (see `documentation/bugs/repo/fixed/2026-09-06-post-commit-catch-up-commit-skips-logging-the-code-commit-it-exists-to-log-SOLVED.md`).
...
python3 tools/wrapup_checklist/run.py --coverage-only     # flag commits with no logged row
```
...
"Recurrence #1" section) — the original fix built a coverage check for `commit_cost` only, and
`doc_metrics` had no equivalent, so the same "logging step quietly drops out of the loop" lapse kept
recurring here specifically, undetected, for about three weeks of commits.

Originally a standalone `check_coverage.sh` shell script (grepping `metrics.jsonl` raw, once per
commit). Retired 2026-08-03 in favor of `tools/wrapup_checklist/`'s shared, tested implementation
(see [WRAPUP_CHECKLIST.md](WRAPUP_CHECKLIST.md)) — one `git log` call plus one parsed pass over the
jsonl, not a per-commit shell-out, and the same code now serves `commit_cost` too instead of two
hand-copied scripts. `--coverage-only` lists any `*.md`-touching commit missing a `metrics.jsonl`
row — except a commit with zero tracked `*.md` files in its tree (this repo's earliest commits,
before any documentation existed), which is a real, expected absence, not a gap. It also runs
automatically, blocking, in `.githooks/pre-commit` — see README.md's wrap-up table — so a gap is
caught at the very next commit, not only when a session remembers to run the full checklist. If it
reports commits missing, `log.py --backfill` catches them up (safe to rerun — dedupes by commit
hash, per the methodology section above).
