...
- `test_main.py` — **not** a pytest test (no `test_*` functions, filename only matches the pytest pattern for a mid-session naming reason). A manual, local-only dev tool: `python3 -m modules.test_main` opens a tkinter window to pick a folder, then a second, scrollable window showing every image in it with its detections' bounding boxes drawn directly on the photo, plus a quality/objects caption.
